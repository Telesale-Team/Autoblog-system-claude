...
PHASES = [
    ...
    },
    {
        ...
        'items': [
            ...
            {'name': 'ถอดตัวเลขที่ไม่จริงออกจากหน้าขาย', 'status': TODO,
             'note': '"50+ โปรเจกต์ที่ส่งมอบแล้ว" กับตัวเลขลอยอีก 2 ตัวใน service_detail.html '
                     'ทั้งที่ลูกค้าจ่ายเงินจริงมี 1 ราย · ความเสี่ยงสูงสุด ต้นทุนต่ำสุด ทำก่อนเรื่องอื่น'},
            ...
            {'name': 'แก้ราคาที่ขัดกันเอง — price_start ของ booking-system 25,000 -> 9,900', 'status': TODO,
             'note': 'ราคาใน DB ไม่ตรงกับหน้าขาย ทำให้เสียเปรียบทันทีที่ prospect ถามราคา'},
            {'name': 'ซ่อน POS + LMS ออกจาก /services/ และแยกกริดตาม delivery_type', 'status': TODO,
             'note': 'แยกเป็น "ระบบพร้อมใช้ — เริ่มได้เลย" กับ "ออกแบบเฉพาะธุรกิจคุณ" '
                     '· ย้ายป้าย "แนะนำ" จาก 3 ตัวที่ยังไม่มีของ ไปที่ QueueFlow'},
            {'name': 'ตัด feature ที่ทำไม่ได้จริงออกจาก DB', 'status': TODO,
             'note': 'SLA 99.5% / Onsite ทั่วประเทศ / Deploy 7 วัน / ROI 3-6 เดือน '
                     '— เขียนเกินของที่มี เสี่ยงถูกเรียกเงินคืน'},

            # --- ข. ชุดหน้าเว็บ V5 แยกบทบาท (มติ QF-1 เจ้าของเคาะ 27 ส.ค. 69) ---
            {'name': 'ออกแบบชุดหน้า V5 โดยทีม agent การตลาด + ขาย + ลอย', 'status': TODO,
            ...
            {'name': 'เปิด reCAPTCHA ที่ pages/forms.py + ปุ่มทำเครื่องหมายสแปม', 'status': TODO,
             'note': '`django_recaptcha` อยู่ใน INSTALLED_APPS แล้วและมี key ใน .env แล้ว '
                     'แต่ ContactForm ไม่ได้เรียกใช้ — ของพร้อมอยู่ในบ้าน แค่ไม่ได้ต่อสาย'},
            {'name': 'ปุ่ม "เพิ่ม Lead เอง" ในหลังบ้าน', 'status': TODO,
             'note': 'GTM สั่ง DM 50 ราย/สัปดาห์ + LINE OA + referral '
                     'แต่ระบบรับ lead ได้ทางเดียวคือฟอร์มบนเว็บ · lead ที่ไปหาเองจึงไม่มีที่ลง'},
            {'name': 'next_follow_up + LeadActivity — คุยอะไร เมื่อไหร่ ครั้งหน้าเมื่อไหร่', 'status': TODO,
             'note': '18 จาก 19 ราย ค้างสถานะ new เพราะไม่มีอะไรทวง'},
            {'name': 'ขยาย SOURCE_CHOICES ให้ตรง 8 ช่องทางของ GTM', 'status': TODO,
             'note': 'linkedin / line_oa / fb_group / youtube / tiktok / referral / cold_dm / google_ads '
                     '· ตอนนี้ source=landing 100% และ utm_source มีค่า 0 จาก 19 '
                     '= ตอบไม่ได้ว่าช่องทางไหนคุ้ม จึงตัดสินใจลงงบไม่ได้'},
            ...
            {'name': 'ซ่อนเมนูกองที่ 2 ให้ sidebar เหลือเฉพาะเมนูที่มีตัวเลขจริง', 'status': TODO,
             'note': 'ซ่อน Quote Builder (ฟอร์มไม่มีช่องใส่ราคา ออกใบจริงไม่ได้) · Customers · Renewals · '
                     ...
                     'ไม่ใช่ศูนย์เรียงกัน'},
            {'name': 'เริ่มลง Expense ทุกรายการตั้งแต่วันนี้', 'status': TODO,
             'note': 'ไม่มีข้อมูลค่าใช้จ่ายจริง = คำนวณ CAC ไม่ได้ = ไม่รู้ว่าช่องทางไหนคุ้ม'},
