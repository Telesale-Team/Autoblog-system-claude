...
CURRENT = {
    ...
    'note': '**27 ส.ค. 2569 เจ้าของสั่งยกเครื่องเรื่องการหาเงิน — เปิดเฟส 9** '
            ...
            'ทั้งหมดรอมือเจ้าของรัน migrate ครั้งเดียว (มติ LEAD-1) ถึงจะใช้ได้จริง '
            '· ยังค้างรอเจ้าของ: RM-3 · PY-1 · LEAD-1',
}
...
PHASES = [
    ...
    },
    {
        ...
        'items': [
            ...
            {'name': 'แก้ราคาที่ขัดกันเอง — price_start ของ booking-system 25,000 -> 9,900', 'status': IN_PROGRESS,
             'note': 'สคริปต์ scripts/fix_service_content.py พร้อมแล้ว สำรองของเดิมก่อนเขียนเสมอ '
            ...
            {'name': 'ซ่อน LMS ออกจาก /services/ และแยกกริดตาม delivery_type', 'status': IN_PROGRESS,
             'note': '**ส่วนที่เป็นโค้ดเสร็จแล้ว 27 ส.ค. 69** — /services/ แยกเป็น '
            ...
            {'name': 'ตัด feature ที่ทำไม่ได้จริงออกจาก DB', 'status': IN_PROGRESS,
            ...
            {'name': 'เปิด reCAPTCHA ที่ pages/forms.py + ปุ่มทำเครื่องหมายสแปม', 'status': IN_PROGRESS,
            ...
            {'name': 'ปุ่ม "เพิ่ม Lead เอง" ในหลังบ้าน', 'status': IN_PROGRESS,
            ...
            {'name': 'next_follow_up + LeadActivity — คุยอะไร เมื่อไหร่ ครั้งหน้าเมื่อไหร่', 'status': IN_PROGRESS,
             'note': '**โค้ดเสร็จ 28 ส.ค. 2569 · รอเจ้าของรัน migrate** '
            ...
            {'name': 'ขยาย SOURCE_CHOICES ให้ตรง 8 ช่องทางของ GTM', 'status': IN_PROGRESS,
             'note': '**โค้ดเสร็จ 28 ส.ค. 2569 · รอเจ้าของรัน migrate** '
...
DECISIONS = [
    ...
    {'code': 'QF-4',
     ...
     'since': date(2026, 8, 28)},
    {'code': 'LEAD-1',
     'topic': 'migration ของงานหลังบ้านรับ lead รอมือเจ้าของรัน — agent รันเองไม่ได้',
     'detail': 'ไฟล์ `pages/migrations/0011_contactlead_created_by_contactlead_is_spam_and_more.py` '
               'สร้างไว้แล้วและเทสผ่านครบบน sqlite (98 เทสเดิม + 27 เทสใหม่) '
               'แต่ฐานข้อมูลของ dev ชี้ไปที่ MySQL peyo_agent ตัวจริงบน 192.168.1.2 '
               'ซึ่ง agent อ่านได้อย่างเดียว เขียนไม่ได้ '
               '· เพิ่ม 5 ช่องบน contactlead (created_by / is_spam / spam_marked_at / '
               'next_follow_up / last_contacted_at) แก้ตัวเลือก source และสร้างตาราง leadactivity '
               '· ทั้ง 5 ช่องเป็น null ได้หรือมีค่า default ครบ ข้อมูลเดิม 19 แถวจึงไม่ถูกแตะ '
               '· **จนกว่าจะรัน หน้า /owner/leads/ จะพังเพราะโค้ดเรียกช่องที่ยังไม่มีใน DB** '
               '· คำสั่ง: `venv\\Scripts\\python.exe manage.py migrate pages` '
               '(หรือรันบนเซิร์ฟเวอร์ตอน deploy รอบถัดไป)',
     'since': date(2026, 8, 28)},
    {'code': 'POS-1',
